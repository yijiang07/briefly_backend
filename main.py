...
def fetch_articles(topics: str, custom_tracking: str):
    # Map common topic labels to simpler search terms
    topic_map = {
    ...
    if topics:
        for t in topics.split(","):
            t = t.strip()
            terms.append(topic_map.get(t, t))
    if custom_tracking:
        for t in custom_tracking.split(","):
            t = t.strip()
            if t and t.upper() != "EMPTY":
                terms.append(t)

    if not terms:
        terms = ["technology", "startups"]

    # Use top 3 terms without quotes so NewsAPI matches more broadly
    ...
    try:
        ...
        data = res.json()
        print(f"  NewsAPI status: {data.get('status')} | total: {data.get('totalResults')} | query: {query}")
